...
/**
 * מוציא את חישוב הגיאומטריה של הלוח על המסך למקום אחד, משותף בין
 * GameWindowMain (משחק מקומי) ל-NetworkGameWindowMain (משחק רשת) - עד
 * עכשיו זה היה קוד פרטי בתוך GameWindowMain, ולכן גם לא נבדק וגם היה
 * נכפל אילו נכתב שוב עבור חלון הרשת. אין כאן שינוי לוגיקה - רק העברה
 * + פרמטריזציה של מה שהיה קבוע (כמו רוחב פאנל הצד) כדי שהמחלקה תשמש
 * את שני החלונות בלי hard-code.
 */
